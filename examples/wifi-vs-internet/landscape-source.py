...
class WifiVsInternet(Scene):
    ...
    def test_card(self, number, title, body, icon, center, width, height):
        ...
        icon.move_to(panel.get_center() + [0, .35, 0])
        t = txt(title, 30 if self.v else 33, TEXT, width=width-.5).move_to(panel.get_center()+[0,-.55,0])
        b = txt(body, 23 if self.v else 26, MUTED, width=width-.5).move_to(panel.get_bottom()+[0,.48,0])
    ...
    def checks(self, duration):
        ...
        self.enter([b,c])
        self.wait(round(((duration - 1.9) - first_wait) * 30) / 30)
        self.leave([h,a,b,c])
